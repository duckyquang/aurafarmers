...
def _digest_A(ledger, k=8):
    ...
    for pid, p in ledger.recent_papers(k):
        ...
        lines.append(f"- [{pid}] {p['body'][:60]} — {p['agent_id']} "
                     f"({cites} citations)")
...
def _parts(cond, agent, ledger, tick, world_events):
...
def inbox(cond, agent, ledger, tick, world_events):
    ...
    text = "\n\n".join(_parts(cond, agent, ledger, tick, world_events))
    other = "A" if cond == "B" else "B"
    other_len = len("\n\n".join(
        _parts(other, agent, ledger, tick, world_events)).split())
